...
export default function Marketplace() {
  ...
  return (
    <div className="animate-fade-in">
      {/* Hero Section */}
      <div className="bg-primary text-white rounded-3xl p-8 mb-10 relative overflow-hidden">
        ...
        <div className="absolute bottom-0 right-20 w-48 h-48 bg-white/5 rounded-full blur-3xl"></div>
      </div>

      {/* Grid */}
      {filteredAssets.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-2xl border border-primary/10 shadow-sm">
          ...
          <p className="text-primary/70">There are currently no verified assets in this category.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredAssets.map((asset) => {
            ...
            return (
            <div key={asset.assetId} className="bg-white rounded-2xl border border-primary/10 shadow-sm hover:shadow-md transition-all overflow-hidden group flex flex-col h-full">
              {/* Image Header */}
              <div className="h-48 bg-surface relative flex items-center justify-center overflow-hidden">
                {imageUrl ? (
                ...
                <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold text-primary flex items-center gap-1 shadow-sm z-10">
                  <ShieldCheck className="w-3 h-3 text-emerald-500" />
                  Verified
                </div>

                {asset.value && (
                   <div className="absolute bottom-4 left-4 bg-primary/95 backdrop-blur-sm px-3 py-1 rounded-lg text-white text-sm font-bold shadow-sm z-10">
              ...
              <div className="p-6 flex-1 flex flex-col">
                ...
                <h3 className="font-bold text-primary text-lg mb-2 line-clamp-2">{asset.description}</h3>
                
                <div className="flex items-start gap-2 text-primary/70 text-sm mb-4 flex-1">
                  <MapPin className="w-4 h-4 mt-0.5 flex-shrink-0" />
                  <span className="line-clamp-1">{asset.jurisdiction === 'PAN' ? 'Panama City, Panama' : asset.jurisdiction}</span>
                </div>

                <div className="border-t border-primary/10 pt-4 flex items-center justify-between mt-auto">
                  <div>
                    <p className="text-xs text-primary/60 font-mono">Owner: {asset.owner.substring(0, 6)}...</p>
                  </div>
